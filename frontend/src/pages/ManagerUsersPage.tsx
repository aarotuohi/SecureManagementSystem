...
import { getAllManagerUsers, updateManagerUser, User } from '../lib/api'
...
export default function ManagerUsersPage() {
  ...
  useEffect(() => {
    async function load() {
      if (!token) return
      const res = await getAllManagerUsers(token)
      if (res.statusCode === 200 && res.ourUsersList) setUsers(res.ourUsersList)
      else setError(res.message || 'Failed to load manager users')
    }
    load()
  }, [token])
  ...
  const save = async (id: number) => {
    ...
    const res = await updateManagerUser(token, id, body || {})
